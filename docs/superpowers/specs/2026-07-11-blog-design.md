...
- **Tema**: nessun tema di terze parti. Layout custom minimale scritto direttamente in questo repo (`layouts/`, `assets/`): home, lista post, singolo post, tassonomia tag, partial di navigazione i18n. Scelta deliberata invece di un tema pubblicato: molti temi Hugo bundlano già una propria toolchain Node/PostCSS per lo styling, il che contraddirebbe la motivazione "zero dipendenze Node" alla base della scelta di Hugo — un layout custom evita il rischio ed è comunque poco codice per un blog di poche viste. Stile con Tailwind CSS per tipografia/colori/identità visiva, tramite il **binario standalone** di Tailwind eseguito come step indipendente **fuori dalla pipeline di Hugo**: da Hugo v0.161.0 la funzione nativa `css.TailwindCSS` richiede Tailwind installato via npm (il supporto al binario standalone al suo interno è stato rimosso), quindi usarla introdurrebbe la toolchain Node che si vuole evitare. Il binario standalone genera un CSS già compilato (`tailwindcss -i assets/css/main.css -o assets/css/tailwind.css --minify`) che i layout Hugo si limitano a servire/fingerprintare: build locale e CI hanno due comandi (Tailwind poi Hugo) invece di uno, ma nessuna dipendenza Node resta nel progetto.
